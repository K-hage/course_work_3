...
@bookmarks_blueprint.route('/')  # /bookmarks/ - страница закладок
def bookmarks_page():
    posts = BookmarksDAO().get_post_bookmarks()
    return render_template("bookmarks.html", posts=posts)


@bookmarks_blueprint.route('/add/<int:post_id>/')  # роут добавления закладки по id
def bookmark_add_page(post_id):
    BookmarksDAO().add_dump_data(post_id)
    return redirect(request.referrer, code=302)


@bookmarks_blueprint.route('/remove/<int:post_id>/')  # роут удаления закладки по id
